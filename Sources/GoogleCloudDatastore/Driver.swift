...
public final class Driver {
    ...
    public private(set) static var `default`: Driver!
    ...
    public static func bootstrap(projectID: String, on eventLoopGroup: EventLoopGroup) -> EventLoopFuture<Driver> {
    ...
    /// - Returns: Future for datastore.
    @discardableResult
    public static func bootstrap(configuration: Configuration, group eventLoopGroup: EventLoopGroup) -> EventLoopFuture<Driver> {

        // Emulator
        if let host = configuration.insecureHost ?? ProcessInfo.processInfo.environment["DATASTORE_EMULATOR_HOST"] {
            let promise = eventLoopGroup.next().makePromise(of: Driver.self)
            do {
    ...
    private static func bootstrapForEmulator(host: String, configuration: Configuration, on eventLoopGroup: EventLoopGroup) throws -> Driver {
        ...
        return Driver(raw: client, projectID: configuration.projectID)
    }

    private static func bootstrapForProduction(configuration: Configuration, on eventLoopGroup: EventLoopGroup) -> EventLoopFuture<Driver> {
        let promise = eventLoopGroup.next().makePromise(of: Driver.self)
        ...
        do {
            try provider.withToken { token, error in
                ...
                promise.succeed(Driver(raw: client, projectID: configuration.projectID))
            }
        } catch {
            promise.fail(error)
        }
    ...
    public func datastore(on eventLoop: EventLoop) -> Datastore {
        Datastore(driver: self, eventLoop: eventLoop)
    }
}
